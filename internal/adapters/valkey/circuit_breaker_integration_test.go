...
func TestCircuitBreaker_TripsAtThreshold(t *testing.T) {
	...
	if cb.State != gateway.StateOpen || cb.CooldownUntil.IsZero() {
		t.Errorf("breaker should be OPEN with cooldown, got state=%s cooldownZero=%v", cb.State, cb.CooldownUntil.IsZero())
	}
}
...
func TestCircuitBreaker_HalfOpenFailureReopens(t *testing.T) {
	...
	if !opened {
		t.Error("failure while HALF_OPEN should immediately re-open the breaker")
	}
}
...
func TestCircuitBreaker_SetLastKnownScorePreservesState(t *testing.T) {
	...
	if cb.ConsecutiveFailures != 2 {
		t.Errorf("score update must not clobber failure count, got %d", cb.ConsecutiveFailures)
	}
	...
	if cb.LastKnownReliabilityScore != 42 {
		t.Errorf("expected score 42, got %d", cb.LastKnownReliabilityScore)
	}
}
