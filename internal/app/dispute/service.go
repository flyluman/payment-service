...
func (s *Service) CreateDispute(ctx context.Context, d *dispute.Dispute) error {
	return s.tx.WithinTx(ctx, func(ctx context.Context) error {
		...
		if existing != nil {
			...
			// WON/LOST resolutions emit DISPUTE_UPDATED from
			// transitionParentOnDisputeUpdate (with the parent's new status);
			// all other updates emit it here, so exactly one event per update.
			if s.outbox != nil && d.Status != dispute.StatusWon && d.Status != dispute.StatusLost {
				payload, err := json.Marshal(map[string]any{
					"dispute_id":     existing.ID.String(),
...
func (s *Service) transitionParentOnDisputeCreate(ctx context.Context, txnID uuid.UUID) error {
	...
	if err := transaction.TransitionState(txn, transaction.StatusDisputed, transaction.ActorGateway); err != nil {
		return fmt.Errorf("dispute: transition parent %s to DISPUTED: %w", txnID, err)
	}
...
func (s *Service) transitionParentOnDisputeUpdate(ctx context.Context, txnID uuid.UUID, disputeStatus dispute.Status) error {
	...
	case dispute.StatusWon:
		if err := transaction.TransitionState(txn, transaction.StatusCaptured, transaction.ActorSystem); err != nil {
			return fmt.Errorf("dispute: transition parent %s to CAPTURED: %w", txnID, err)
		}
	...
	case dispute.StatusLost:
		if err := transaction.TransitionState(txn, transaction.StatusRefunded, transaction.ActorSystem); err != nil {
			return fmt.Errorf("dispute: transition parent %s to REFUNDED: %w", txnID, err)
		}
		if err := s.txns.UpdateStatus(ctx, txn); err != nil {
			return err
		}
...
func (s *Service) GetDispute(ctx context.Context, tenantID, id uuid.UUID) (*dispute.Dispute, error) {
	d, err := s.store.GetByID(ctx, id)
	if err != nil {
		return nil, err
	}
	if !s.tenantCanAccess(ctx, tenantID, d) {
...
func (s *Service) SubmitEvidence(ctx context.Context, tenantID, disputeID uuid.UUID, ev *dispute.Evidence) error {
...
func (s *Service) ListEvidence(ctx context.Context, tenantID, disputeID uuid.UUID) ([]*dispute.Evidence, error) {
	d, err := s.store.GetByID(ctx, disputeID)
	if err != nil {
		return nil, err
	}
...
func (s *Service) tenantCanAccess(ctx context.Context, tenantID uuid.UUID, d *dispute.Dispute) bool {
	if tenantID == uuid.Nil {
		return true
	}
	txn, err := s.txns.GetByID(ctx, d.TransactionID)
	if err != nil || txn == nil {
		return false
	}
	return txn.TenantID == tenantID
}
